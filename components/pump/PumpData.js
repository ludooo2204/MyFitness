...
import { deleteSpecificElementTablePump } from '../../Services/deleteSpecificElementTable';
const PumpData = ({ navigation }) => {
    const [data, setData] = useState(null)
    const [modalVisible, setModalVisible] = useState(false);
    const [selectedSession, setSelectedSession] = useState(null);

    useEffect(() => {
        ReadPumpFromDatabase(e => {
            setData(e)
        });
    }, [])
    let dataForGraph2;
    ...
    const confirmDelete = () => {
        ...
        deleteSpecificElementTablePump(selectedSession)
        ReadPumpFromDatabase(e => {
            setData(e)
        });
    }
    ...
    return (<View style={{ flex: 1 }}>
        ...
        <ScrollView style={{ flex: 1 }}>{data && data.map((e, i) => {
            return <GainageSessionContainer style={{
                ...
                <GainageSessionText  >{e.count} pompes   --  {new Date(e.date).toLocaleString('fr-FR')}</GainageSessionText>
